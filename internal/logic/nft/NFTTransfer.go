...
import (
	...
	"nez-server/internal/logic/contract/nft"
...
func NewNFT() service.INFT {
	return &sNFT{ContractAddresses: global.NFTs}
}

// ConsumeEvents 消费链上事件
func (s *sNFT) ConsumeEvents(ctx context.Context) error {
	for _, contractAddress := range s.ContractAddresses {
		if logs, err := consumer.ConsumeAllEvent(ctx, contractAddress.String()); err != nil {
			return err
		} else {
			ids := make([]uint64, 0)
			for _, log := range logs {
				err := g.DB().Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
					return s.handleEventLog(ctx, contractAddress, &log)
				})
				if err != nil {
...
func (s *sNFT) handleEventLog(ctx context.Context, contractAddress common.Address, log *scanner.Elog) error {
	...
	g.Log().Infof(ctx, "Handle nft transfer", event.From.Hex(), event.To.Hex(), event.TokenId.Int64())
	return s.newNFTTransfer(ctx, contractAddress, event, log)
}

func (s *sNFT) newNFTTransfer(ctx context.Context, contractAddress common.Address, transfer *nft.NftTransfer, log *scanner.Elog) error {
	hash := log.TxHash.Hex()
	...
	return service.NFTHold().NewTransfer(ctx, contract, to, tokenId, time.Unix(int64(eventLogTime), 0))
}
